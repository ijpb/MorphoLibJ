 ...
 *
 */
public class LabelGeodesicDistanceTransform5x5FloatScanningTest
{
	/**
	...
	public final void testGeodesicDistanceMap_SingleRect_Borgefors()
	{
		...
		assertEquals(0, map.getf(3, 2), .1);
		assertEquals(3, map.getf(4, 2), .1);
		assertEquals(4, map.getf(4, 3), .1);
		
		assertEquals(7, map.getf(1, 1), .1);
		assertEquals(17, map.getf(8, 4), .1);
	}
	...
	public final void testGeodesicDistanceMap_SingleRect_ChessKnight()
	{
		...
		LabelGeodesicDistanceTransform algo = new LabelGeodesicDistanceTransform5x5FloatScanning(weights, false);
		
		ImageProcessor map = algo.geodesicDistanceMap(marker, mask);
		
		assertEquals(0, map.getf(3, 2), .1);
		assertEquals(5, map.getf(4, 2), .1);
		assertEquals(7, map.getf(4, 3), .1);
		
		assertEquals(11, map.getf(1, 1), .1);
		assertEquals(27, map.getf(8, 4), .1);
	}
	...
	public final void testGeodesicDistanceMap_Hilbert_Borgefors()
	{
		...
		assertEquals(0, map.getf(1, 4), .1);
		assertEquals(111, map.getf(1, 7), .1);
	}
	...
	public final void testGeodesicDistanceMap_Hilbert_ChessKnight()
	{
		...
		assertEquals(0, map.getf(1, 4), .1);
		assertEquals(177, map.getf(1, 7), .1);
	}
	...
	public final void testGeodesicDistanceMap_ChessKnight()
	{
		...
		assertEquals(250.8, map.getf(190, 210), .1);
	}
